        ...
        for item in data:
            ...
            if "system_prompt" in item:
                conversation["conversations"].insert(
                    0, {"from": "system", "value": item["system_prompt"]}
                )
...
class ArgillaExporter:
    ...
    def push_to_argilla(
        ...
        Args:
            ...
            workspace: Argilla workspace name (default: argilla)
        ...
        if client is None:
            raise ConnectionError(f"Failed to connect to Argilla at {self.api_url}")

        # Ensure workspace exists (auto-create if not)
        ws = client.workspaces(name=workspace)
        if ws is None:
            ws = rg.Workspace(name=workspace, client=client)
            ws.create()
        ...
        # Return user-accessible URL (always use localhost for browser access)
        external_url = self.api_url.replace("http://argilla:6900", "http://localhost:6900")
        return f"{external_url}/dataset/{dataset.id}/annotation-mode"
